...
def get_dep_add_lines_bigvul(cache=True, splits="default"):
    """Cache dependent added lines for bigvul."""
    # saved = utils.get_dir(utils.processed_dir() / "bigvul/eval") / "statement_labels.pkl"
    # 在缓存文件名中加入splits参数，确保不同划分使用不同缓存
    saved = utils.get_dir(utils.processed_dir() / "bigvul/eval") / f"statement_labels_{splits}.pkl"
    if os.path.exists(saved) and cache:
        with open(saved, "rb") as f:
            return pkl.load(f)
    # df = bigvul()
    df = bigvul(splits=splits,minimal=False)
    df = df[df.vul == 1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--splits", type=str, default="default", help="data split mode, e.g., default or crossval-5")
    parser.add_argument("--no_cache", action="store_true", help="ignore cached .pkl file")
    args = parser.parse_args()

    # ✅ 显式传入 splits 参数
    get_dep_add_lines_bigvul(cache=not args.no_cache, splits=args.splits)
