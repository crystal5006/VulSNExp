...
class CFExplainer(ExplainerBase):
    ...
    def forward(self, x, edge_index, batch, mask_features=False, target_label=None, **kwargs):
        r"""
        ...
        if kwargs.get('edge_masks'):
            ...
            for ex_label in ex_labels:
                if target_label is None or ex_label.item() == target_label.item():
                    ...
                    self.__set_masks__(x, self_loop_edge_index)
                    edge_mask = self.gnn_explainer_alg(x, edge_index, ex_label,batch=batch).sigmoid()
