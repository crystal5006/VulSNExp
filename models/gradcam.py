...
class GradCAM(WalkBase):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, **kwargs) \
        ...
        if kwargs.get('edge_masks'):
            ...
            self.explain_method = GraphLayerGradCam(model, model.gnn_layers[-1])
            # --- setting end ---
            ...
            for ex_label in ex_labels:
                # 总是传递 batch 参数，如果没有则创建一个默认的
                if batch is None:
                    # 为单个图创建一个默认的 batch
                    batch = torch.zeros(x.shape[0], dtype=torch.long, device=x.device)
                attr_wo_relu = self.explain_method.attribute(x, ex_label,
                                                             additional_forward_args=(edge_index, batch)).detach()
        ...
        with torch.no_grad():
            with self.connect_mask(self):
                # 确保 batch 参数被正确传递，过滤掉解释器特有的参数
                filtered_kwargs = {k: v for k, v in kwargs.items()
                                   if k not in ['sparsity', 'num_classes', 'node_idx', 'edge_masks']}
                # 确保 batch 参数存在
                if 'batch' not in filtered_kwargs and batch is not None:
                    filtered_kwargs['batch'] = batch
