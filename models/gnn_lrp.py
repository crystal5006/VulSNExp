...
class GNN_LRP(WalkBase):
    ...
    def extract_step(self, x: Tensor, edge_index: Tensor, batch: Tensor, detach: bool = True, split_fc: bool = False):
    ...
    def forward(self,
        ...
        # 获取 batch 参数，如果没有则创建一个默认的
        batch = kwargs.get('batch')
        if batch is None:
            batch = torch.zeros(x.shape[0], dtype=torch.long, device=x.device)

        transform_steps, walk_steps, fc_steps = self.extract_step(x, edge_index, batch, detach=False, split_fc=True)
                ...
                for walk_indices in walk_indices_list:
                    ...
                    for i, fc_step in enumerate(fc_steps):
                        ...
                        std_h = nn.Sequential(*modules)(h) if i != 0 \
                            else GraphSequential(*modules)(h, torch.zeros(h.shape[0], dtype=torch.long,
                                                                          device=self.device))
        ...
        with torch.no_grad():
            with self.connect_mask(self):
                ...
                # 过滤掉解释器特有的参数，只保留模型需要的参数
                filtered_kwargs = {k: v for k, v in kwargs.items()
                                  if k not in ['sparsity', 'num_classes', 'node_idx', 'edge_masks']}
                related_preds = self.eval_related_pred(x, edge_index, hard_edge_masks, **filtered_kwargs)
