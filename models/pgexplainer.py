...
import torch
from dig.xgraph.method.pgexplainer import calculate_selected_nodes
...
class XPGExplainer(PGExplainer):
    ...
    def explain(self,
        ...
        probs = self.model(x, edge_index, batch)
        ...
        if self.explain_graph:
            with torch.no_grad():
                ...
                for gid in tqdm.tqdm(dataset_indices):
                    ...
                    logits = self.model(data.x, data.edge_index, batch=data.batch)
            ...
            for epoch in range(self.epochs):
                ...
                for gid in tqdm.tqdm(dataset_indices):
                    data = dataset[gid]
                    data.to(self.device)
                    prob, edge_mask = self.explain(data.x, data.edge_index, batch=data.batch, embed=emb_dict[gid], tmp=tmp, training=True)
...
class PGExplainer_edges(ExplainerBase):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, batch: Tensor = None, **kwargs) -> Tuple[List, List, List[Dict]]:
        # set default subgraph with 10 edges
        ...
        if self.explain_graph:
            ...
            with torch.no_grad():
                if self.explain_graph:
                    related_preds = self.eval_related_pred(x, edge_index, hard_edge_masks, batch=batch)
            ...
            with torch.no_grad():
                related_preds = self.eval_related_pred(
                    subgraph_x, subgraph_edge_index, hard_edge_masks, node_idx=self.new_node_idx, batch=batch)
