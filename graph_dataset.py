...
class VulGraphDataset(Dataset):
    ...
    def process(self):
        ...
        self.df = bigvul(splits=self.splits, minimal=False)#强制刷新划分，非常关键的一步
        # print("df shape after bigvul:", self.df.shape)
        # print("label unique after bigvul:", self.df['label'].unique())
        # print("label value counts after bigvul:\n", self.df['label'].value_counts())

        # 如果是交叉验证的标签（如 val_0, test_0），则正常处理
        if self.partition.startswith("val_") or self.partition.startswith("test_") or self.partition == "train":
            self.df = self.df[self.df.label == self.partition]
        else:
            raise ValueError(f"Unsupported partition label: {self.partition}")
        self.df["id"] = self.df["id"].astype(int)
        self.df = self.df[self.df.id.isin(self.finished)]
        print("finished count:", len(self.finished))
        print("df count before filter:", len(self.df))
        ...
        print("after valid:", self.df.columns, self.df.shape)
        print(self.df.head())
        # Get mapping from index to sample ID.
        self.df = self.df.reset_index(drop=True).reset_index()
        print("after reset_index:", self.df.columns, self.df.shape)
        print(self.df.head())
...
if __name__ == '__main__':
    ...
    # 新增
    splits = sys.argv[2] if len(sys.argv) > 2 else 'default'
    ...
    dataset = VulGraphDataset(root=str(utils.processed_dir() / "vul_graph_dataset"), encoder=language_model,
                              tokenizer=tokenizer, partition=partition, splits=splits)
